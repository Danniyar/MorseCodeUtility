...
const sentenceElm = document.getElementById('sentence');
...
{
    beepstop();
    checkID = setTimeout(check, dotDuration*20);
...
function start(){
    if(curSentence == undefined)
    {
        ...
        sentenceElm.textContent = curSentence;
        curSentenceMorse = '';
        for(var i = 0; i < curSentence.length; i++)
        {
            if(curSentence[i] == ' ')
                continue;
            curSentenceMorse += letterlibrary[curSentence[i]] + ' ';
        }
        curSentenceMorse = curSentenceMorse.slice(0, -1);
        heartsElm.innerHTML = '';
        for(var i = 0; i < lives; i++)
            heartsElm.innerHTML += heartSvg;
        inputarea.innerHTML = '&nbsp;';
        inputarea.style.color = 'white';
        sentenceElm.style.color = 'white';
...
function check(){
    ...
    if(inputarea.textContent == curSentenceMorse)
    {
        correct += 1;
        sentenceElm.textContent = 'Correct';
        inputarea.style.color = 'cyan';
        sentenceElm.style.color = 'cyan';
        sentenceElm.style.animation = 'none';
        sentenceElm.offsetHeight;
        sentenceElm.style.animation = null;
        sentenceElm.style.animation = 'vertical-shaking 0.7s';
        setTimeout(unfreeze, 1000);
    }
    else
    {
        sentenceElm.innerHTML = 'Wrong';
        inputarea.textContent = curSentenceMorse;
        inputarea.style.color = 'red';
        sentenceElm.style.color = 'red';
        lives -= 1;
        heartsElm.innerHTML = '';
        for(var i = 0; i < lives; i++)
            heartsElm.innerHTML += heartSvg;
        ...
        sentenceElm.offsetHeight;
        sentenceElm.style.animation = null;
        sentenceElm.style.animation = 'horizontal-shaking 0.7s';
